...
def call_deep_config_chunk_api(chunk_params: dict):
    ...
    try:
        ...
        selected_numeric_columns = chunk_params.get("selected_numeric_columns", [])
        selected_categorical_columns = chunk_params.get("selected_categorical_columns", [])
        ...
        data = {
            ...
            "selected_numeric_columns": json.dumps(selected_numeric_columns),
            "selected_categorical_columns": json.dumps(selected_categorical_columns)
        }
        if key_column:
...
def download_preprocessed():
    """Download preprocessed data (for Fast Mode and Config-1 Mode)"""
    ...
    if st.session_state.current_mode == "fast":
        ...
        if uploaded_file or (input_source == "🗄️ Database Import" and use_db_config):
                    ...
                    with st.spinner("🔄 Running preprocessing via API..."):
                        try:
                            ...
                            if "error" in result:
                                ...
                                # Update deep_df with preprocessed data from download API
                                try:
                                    # Get preprocessed data from download API
                                    download_response = requests.get(f"{API_BASE_URL}/export/preprocessed")
                                    if download_response.status_code == 200:
                                        ...
                                        preprocessed_df = pd.read_csv(tmp_path)
                                        st.session_state.deep_df = preprocessed_df
                                        st.success("📊 Data preview updated with preprocessed data!")
                                        
                                        # Clean up
                                        os.unlink(tmp_path)
                                    else:
                                        st.warning("⚠️ Could not fetch preprocessed data")
                                except Exception as e:
                                    st.warning(f"⚠️ Could not update data preview: {str(e)}")
            ...
            if st.session_state.deep_config_step == 1:
                ...
                # Display current data types overview
                st.write("📊 **Current Data Types Overview:**")
                type_df = pd.DataFrame({
                    'Column': st.session_state.deep_df.columns,
                    'Current Type': [str(dtype) for dtype in st.session_state.deep_df.dtypes],
                    'Sample Values': [str(st.session_state.deep_df[col].dropna().iloc[0]) if not st.session_state.deep_df[col].dropna().empty else 'N/A' for col in st.session_state.deep_df.columns]
                })
                ...
                for current_type, columns in dtype_groups.items():
                    ...
                    # Get the most common current type for this group
                    group_types = [str(st.session_state.deep_df[col].dtype) for col in columns]
                    most_common_type = max(set(group_types), key=group_types.count)
                    ...
                    target_type = st.selectbox(
                        ...
                        options,
                        index=default_map.get(current_type, 0),
                    ...
                    if target_type != "No change":
                        ...
                        if selected_columns:
                            st.info(f"✅ Selected: {target_type} for {', '.join(selected_columns)}")
            ...
            if st.session_state.deep_config_step == 2:
                ...
                # Null analysis overview
                profile = profile_nulls_enhanced(st.session_state.deep_df)
                filtered_profile = profile[profile['null_count'] > 0].copy()
                
                # Create column name mapping for API calls (normalize column names)
                column_mapping = {}
                for col in st.session_state.deep_df.columns:
                    normalized_name = col.lower().replace(' ', '_').replace('-', '_')
                ...
                if filtered_profile.empty:
                    ...
                    st.dataframe(
                        filtered_profile[['column','dtype','null_count','null_pct']].sort_values('null_pct', ascending=False),
                        use_container_width=True,
                        height=250
                    )
                ...
                def get_null_options_for_column(col_name, col_data):
                    """Get appropriate null handling options based on column data type"""
                    dtype = str(col_data.dtype)
                    
                    if dtype in ['int64', 'float64']:
                        return ["No change", "drop", "mean", "median", "mode", "zero", "ffill", "bfill", "Custom Value"]
                    elif dtype == 'object':
                        # Check if categorical (low cardinality)
                        if col_data.nunique() / len(col_data) < 0.1:
                            return ["No change", "drop", "mode", "unknown", "ffill", "bfill", "Custom Value"]
                        else:
                            return ["No change", "drop", "mode", "unknown", "ffill", "bfill", "Custom Value"]
                    elif dtype.startswith('datetime'):
                        return ["No change", "drop", "ffill", "bfill", "Custom Value"]
                    elif dtype == 'bool':
                        return ["No change", "drop", "mode", "ffill", "bfill", "Custom Value"]
                    else:
                        return ["No change", "drop", "mode", "unknown", "ffill", "bfill", "Custom Value"]
                
                def get_strategy_descriptions():
                    """Get descriptions for null handling strategies"""
                    return {
                ...
                for i in range(0, len(filtered_profile), cols_per_row):
                    ...
                    for j, (_, row) in enumerate(filtered_profile.iloc[i:i+cols_per_row].iterrows()):
                        with cols[j]:
                            ...
                            # Get appropriate options for this column
                            col_data = st.session_state.deep_df[col_name]
                            options = get_null_options_for_column(col_name, col_data)
                            descriptions = get_strategy_descriptions()
                            
                            # Display column info
                            st.write(f"**{col_name}** ({row['dtype']})")
                            st.write(f"Nulls: {null_count} ({null_pct:.1f}%)")
                            
                            # Get current selection
                            normalized_name = column_mapping[col_name]
                            current_selection = null_strategies.get(normalized_name, "No change")
                            current_index = options.index(current_selection) if current_selection in options else 0
                            
                            # Create options with descriptions for tooltip
                            options_with_desc = []
                            for option in options:
                                desc = descriptions.get(option, "")
                                if desc:
                                    options_with_desc.append(f"{option} - {desc}")
                                else:
                                    options_with_desc.append(option)
                            ...
                            selected_desc = descriptions.get(choice, "")
                            if selected_desc:
                                st.info(f"💡 **{choice}**: {selected_desc}")
                            
                            # Handle custom value input
                            custom_value = None
                            if choice == "Custom Value":
                                st.write("**Enter custom value:**")
                                custom_value = st.text_input(
                                    f"Custom value for {col_name}:",
                                    key=f"deep_custom_value_{col_name}",
                                    placeholder="Enter value to fill nulls..."
                                )
                                # Always store custom value, even if empty (will be treated as empty string)
                                if custom_value is not None:  # Check if input field exists
                                    # Store as "custom_value:actual_value" format
                                    choice = f"custom_value:{custom_value}"
                            
                            if choice != "No change":
                                normalized_name = column_mapping[col_name]
                                null_strategies[normalized_name] = choice
                            elif column_mapping[col_name] in null_strategies:
                                normalized_name = column_mapping[col_name]
                                del null_strategies[normalized_name]
                    ...
                    with st.spinner("🔄 Applying null handling via API..."):
                        try:
                            ...
                            if "error" in result:
                                ...
                                # Update deep_df with processed data from download API
                                try:
                                    download_response = requests.get(f"{API_BASE_URL}/export/preprocessed")
                                    if download_response.status_code == 200:
                                        import tempfile
                                        ...
                                        preprocessed_df = pd.read_csv(tmp_path)
                                        st.session_state.deep_df = preprocessed_df
                                        st.success("📊 Data preview updated with null handling results!")
                                        
                                        os.unlink(tmp_path)
                                    else:
                                        st.warning("⚠️ Could not fetch processed data")
                                except Exception as e:
                                    st.warning(f"⚠️ Could not update data preview: {str(e)}")
            ...
            if st.session_state.deep_config_step == 3:
                ...
                with col2:
                    st.metric("Duplicate Pairs", duplicate_analysis['duplicate_pairs_count'])
                with col3:
                    st.metric("Duplicate %", f"{duplicate_analysis['duplicate_percentage']:.1f}%")
                
                if duplicate_analysis['has_duplicates']:
                    st.warning(f"⚠️ Found {duplicate_analysis['duplicate_pairs_count']} duplicate pairs")
                    
                    # Simple duplicate handling options
                    st.write("🛠️ **Duplicate Handling:**")
                    strategy = st.radio(
                    ...
                    if strategy == "Remove Duplicates":
                        total_rows = duplicate_analysis['total_rows']
                        duplicate_pairs = duplicate_analysis['duplicate_pairs_count']
                        remaining_rows = total_rows - duplicate_pairs
                        st.warning(f"⚠️ Removes duplicate rows, keeping only the first occurrence (e.g., {total_rows} rows → {remaining_rows} rows if {duplicate_pairs} duplicates found)")
                    
                    # Map strategy to backend values
                    if strategy == "Remove Duplicates":
                        selected_strategy = "keep_first"  # Use keep_first as default for "Remove Duplicates"
                    else:
                        selected_strategy = "keep_all"  # Skip means keep all
                    
                    # Apply button
                    if st.button("Apply Changes", key="deep_apply_duplicates"):
            ...
            if st.session_state.deep_config_step == 4:
                ...
                if text_cols.empty:
                    ...
                    if choice == "Apply stop word removal":
                        st.warning("⚠️ Removes common words like 'the', 'and', 'is' (e.g., 'the customer is satisfied' → 'customer satisfied')")
                                ...
                                if "error" in result:
                                    ...
                                    try:
                                        ...
                                        if download_response.status_code == 200:
                                            ...
                                            preprocessed_df = pd.read_csv(tmp_path)
                                            st.session_state.deep_df = preprocessed_df
                                            st.success("📊 Data preview updated with stopwords processing results!")
            ...
            if st.session_state.deep_config_step == 5:
                ...
                if choice == "Apply lemmatization":
                    st.info("ℹ️ Converts words to dictionary forms (e.g., 'amazing' → 'amaze')")
                elif choice == "Apply stemming":
                    st.warning("⚠️ Aggressively reduces words to root forms (e.g., 'amazing' → 'amaz')")
                            ...
                            if "error" in result:
                                ...
                                try:
                                    ...
                                    if download_response.status_code == 200:
                                        ...
                                        preprocessed_df = pd.read_csv(tmp_path)
                                        st.session_state.deep_df = preprocessed_df
                                        st.success("📊 Data preview updated with text normalization results!")
...
if export_ready:
    ...
    with col1:
        ...
        """, unsafe_allow_html=True)
        if st.button("📄 Export Preprocessed CSV", use_container_width=True):
            try:
                if st.session_state.current_mode == "deep":
                    csv_data = download_deep_config_preprocessed()
                else:
