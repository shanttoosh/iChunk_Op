# backend.py - MEDIA CAMPAIGN ONLY VERSION WITH JSON FIXES + RETRIEVAL STRUCTURE FIX
import pandas as pd
import numpy as np
import re
import time
...
current_preprocessed_df = None
current_distance_metric = "cosine"
current_media_campaign_data = None
current_contact_records = None
...
# -----------------------------
# 🔹 Media Campaign Functions - ENHANCED
# -----------------------------
...
def preprocess_media_campaign(df: pd.DataFrame, field_mapping: Dict[str, str] = None) -> pd.DataFrame:
    ...
    global current_preprocessed_df, current_media_campaign_data
    current_preprocessed_df = df_clean.copy()
    current_media_campaign_data = {
...
# -----------------------------
def chunk_media_campaign_records(df: pd.DataFrame, records_per_chunk: int = 5, field_mapping: Dict[str, str] = None) -> Tuple[List[str], List[Dict]]:
    ...
    for idx, row in df.iterrows():
        ...
        structured_records.append({
            ...
            'data': record_data,
            'index': int(idx)  # Convert to native int
        })
...
def chunk_media_campaign_by_company(df: pd.DataFrame, field_mapping: Dict[str, str] = None, max_records_per_company: int = 10) -> Tuple[List[str], List[Dict]]:
    ...
    if company_cols:
        # Prefer 'company_name' over 'company'
    ...
    if company_col is None:
        ...
        return chunk_media_campaign_records(df, 5, field_mapping)
...
def chunk_media_campaign_by_source(df: pd.DataFrame, field_mapping: Dict[str, str] = None, max_records_per_source: int = 8) -> Tuple[List[str], List[Dict]]:
...
# NEW: Semantic Clustering Chunking for Media Campaign
def chunk_media_campaign_semantic(df: pd.DataFrame, field_mapping: Dict[str, str] = None, n_clusters: int = 10) -> Tuple[List[str], List[Dict]]:
    """Apply semantic clustering to media campaign data for intelligent grouping"""
    start_time = time.time()
    from sentence_transformers import SentenceTransformer
    
    # Convert contact records to meaningful text
    contact_texts = []
    for idx, row in df.iterrows():
        text_parts = []
        for col, field_type in field_mapping.items():
...
# NEW: Document-based Chunking for Media Campaign
def chunk_media_campaign_document(df: pd.DataFrame, key_column: str, field_mapping: Dict[str, str] = None, token_limit: int = 2000) -> Tuple[List[str], List[Dict]]:
...
# -----------------------------
# 🔹 SMART COMPANY RETRIEVAL SYSTEM - FIXED JSON SERIALIZATION
# -----------------------------
def smart_company_retrieval(query: str, search_field: str = "auto", k: int = 5, include_complete_records: bool = True):
    """
    SMART TWO-STAGE RETRIEVAL:
    Stage 1: Exact/Partial company matching
    Stage 2: Semantic fallback
    """
    global current_media_campaign_data, current_chunks, current_model, current_store_info
    
    logger.info(f"🎯 SMART Company Retrieval - Query: '{query}', Field: {search_field}")
    
    start_time = time.time()
    
    # If no media campaign data, fall back to regular retrieval
    if not current_media_campaign_data:
        logger.warning("No media campaign data found, using regular retrieval")
        return retrieve_media_campaign(query, search_field, k, include_complete_records)
    
    field_mapping = current_media_campaign_data.get('field_mapping', {})
    processed_df = current_media_campaign_data.get('processed_df')
    ...
    semantic_results = retrieve_media_campaign(query, search_field, k, include_complete_records)
...
def find_company_matches(df: pd.DataFrame, field_mapping: Dict[str, str], query: str) -> List[Dict]:
    ...
    for company_col in company_cols:
        ...
        for idx in exact_indices:
            # Convert row to dict and serialize numpy types
            ...
            exact_matches.append({
                ...
                'match_type': 'exact',
                'similarity': 1.0,
                'record_index': int(idx)  # Convert to native int
            })
        ...
        if not exact_mask.any():
            ...
            for idx in partial_indices:
                ...
                record = {k: convert_to_serializable(v) for k, v in record_dict.items()}
                company_value = record[company_col]
                # Calculate partial match similarity
                ...
                partial_matches.append({
                    ...
                    'match_type': 'partial',
                    'similarity': float(similarity),  # Convert to native float
                    'record_index': int(idx)  # Convert to native int
                })
...
def format_company_matches(company_matches: List[Dict], query: str, k: int, include_complete_records: bool) -> Dict:
    ...
    for i, match in enumerate(company_matches[:k]):
        ...
        result_data = {
            ...
            'similarity': float(match['similarity']),  # Convert to native float
...
def store_faiss(embeddings):
    ...
    index = faiss.IndexFlatIP(d)  # Changed to Inner Product for cosine similarity
...
# -----------------------------
# 🔹 Retrieval Functions - FIXED JSON SERIALIZATION + CONSISTENT STRUCTURE
# -----------------------------
def retrieve_media_campaign(query: str, search_field: str = "all", k: int = 5, include_complete_records: bool = True):
    """Specialized retrieval for media campaign data with CONSISTENT result structure"""
    global current_model, current_store_info, current_chunks, current_embeddings, current_media_campaign_data
    ...
    if current_chunks is None or len(current_chunks) == 0:
    ...
    try:
        if current_store_info["type"] == "faiss":
            ...
            # Search with inner product (cosine similarity)
            ...
            for i in range(len(indices[0])):
                ...
                if idx < len(current_chunks):
                    ...
                    result_data = {
                        ...
                        "similarity": float(similarity_val),  # Convert to native float
                        "distance": float(1 - similarity_val),  # Convert to native float
                        "metric": "cosine"
                    }
                    
                    # Add complete records if available and requested
                    if include_complete_records and current_media_campaign_data:
            ...
            for i, (doc, distance) in enumerate(zip(
                ...
                result_data = {
                    ...
                    "content": doc,
                    "similarity": float(similarity),  # Convert to native float
                    "distance": float(distance),  # Convert to native float
                    "metric": "cosine"
                }
                
                # Add complete records if available and requested
                if include_complete_records and current_media_campaign_data:
                    # Find the chunk index that matches this document
...
def get_complete_records_for_chunk(chunk_idx: int) -> List[Dict]:
    ...
    global current_media_campaign_data
    
    if not current_media_campaign_data:
        return []
    
    metadata = current_media_campaign_data.get('metadata', [])
...
# -----------------------------
# 🔹 Pipeline Functions - MEDIA CAMPAIGN ONLY
# -----------------------------
def run_media_campaign_pipeline(df, chunk_method="record_based", chunk_size=5, model_choice="paraphrase-MiniLM-L6-v2", 
    ...
    global current_model, current_store_info, current_chunks, current_embeddings, current_df, current_media_campaign_data
    
    logger.info(f"🎯 Starting Media Campaign pipeline with {len(df)} records")
    current_df = df.copy()
    set_file_info(file_info)
    
    field_mapping = detect_media_campaign_fields(df)
    df1 = preprocess_media_campaign(df, field_mapping)
    ...
    if chunk_method == "record_based":
        chunks, metadata = chunk_media_campaign_records(df1, chunk_size, field_mapping)
    elif chunk_method == "company_based":
        chunks, metadata = chunk_media_campaign_by_company(df1, field_mapping, chunk_size)
    elif chunk_method == "source_based":
        chunks, metadata = chunk_media_campaign_by_source(df1, field_mapping, chunk_size)
    elif chunk_method == "semantic_clustering":
        chunks, metadata = chunk_media_campaign_semantic(df1, field_mapping, chunk_size)
    elif chunk_method == "document_based":
        if document_key_column:
            chunks, metadata = chunk_media_campaign_document(df1, document_key_column, field_mapping)
        else:
            # Auto-select document key
            ...
            chunks, metadata = chunk_media_campaign_document(df1, doc_key, field_mapping)
            logger.info(f"Auto-selected document key: {doc_key}")
    else:
        chunks, metadata = chunk_media_campaign_records(df1, 5, field_mapping)
    ...
    if storage_choice == "faiss":
        ...
        store = store_chroma(chunks, embs, f"media_campaign_{int(time.time())}")
    ...
    logger.info(f"✅ Media Campaign pipeline completed: {len(chunks)} chunks created")
...
# -----------------------------
# 🔹 Large File Processing for Media Campaign
# -----------------------------
...
def process_large_file_media_campaign(file_path: str, **kwargs):
    """Process large media campaign file in batches"""
    def process_batch(batch_df):
        field_mapping = detect_media_campaign_fields(batch_df)
        processed_df = preprocess_media_campaign(batch_df, field_mapping)
        ...
        if chunk_method == "record_based":
            chunks, _ = chunk_media_campaign_records(processed_df, chunk_size, field_mapping)
        elif chunk_method == "company_based":
            chunks, _ = chunk_media_campaign_by_company(processed_df, field_mapping, chunk_size)
        elif chunk_method == "source_based":
            chunks, _ = chunk_media_campaign_by_source(processed_df, field_mapping, chunk_size)
        elif chunk_method == "semantic_clustering":
            chunks, _ = chunk_media_campaign_semantic(processed_df, field_mapping, chunk_size)
        elif chunk_method == "document_based":
            if document_key_column:
                chunks, _ = chunk_media_campaign_document(processed_df, document_key_column, field_mapping)
            else:
                possible_keys = [col for col, ftype in field_mapping.items() if ftype in ['company', 'company_name']]
                doc_key = possible_keys[0] if possible_keys else processed_df.columns[0]
                chunks, _ = chunk_media_campaign_document(processed_df, doc_key, field_mapping)
        else:
            chunks, _ = chunk_media_campaign_records(processed_df, 5, field_mapping)
    ...
    if storage_choice == "faiss":
        ...
        store = store_chroma(all_chunks, embs, f"media_campaign_large_{int(time.time())}")
    ...
    logger.info(f"✅ Large file Media Campaign pipeline completed: {len(all_chunks)} chunks created")
...
def process_file_direct(file_path: str, **kwargs):
    ...
    return process_large_file_media_campaign(file_path, **kwargs)
...
def export_chunks():
    ...
    if current_chunks:
        chunks_df = pd.DataFrame({
            'chunk_id': range(len(current_chunks)),
            'chunk_content': current_chunks
        })
...
def export_embeddings():
    ...
    if current_embeddings is not None:
        embeddings_list = current_embeddings.tolist()
        return {
            "embeddings": embeddings_list,
            "shape": list(current_embeddings.shape),  # Convert to list
